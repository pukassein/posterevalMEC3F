...
const startIndex = code.indexOf('<div className="mt-4 mb-8 pt-2 pb-6 border-b border-slate-100 text-left">');
const endIndex = code.indexOf('{/* TAB: CRITERIA */}');

if (startIndex !== -1 && endIndex !== -1) {
  const newWorksUi = `              <div className="mt-4 mb-8 pt-2 pb-6 border-b border-slate-100 text-left">
                <button
                  type="button"
              ...
              <div className="space-y-8 max-w-4xl text-left">
                {sortedWorks.length === 0 && (
                  <p className="text-slate-500 italic text-sm">Nenhum trabalho cadastrado com esses filtros.</p>
                )}

                {sortedWorks.filter(w => w.type === 'poster').length > 0 && (
                  <div>
                    ...
                    <div className="space-y-4">
                      {sortedWorks.filter(w => w.type === 'poster').map(work => (
                ...
                {sortedWorks.filter(w => w.type === 'oral').length > 0 && (
                  <div>
                    <h3 className="text-lg font-bold text-slate-900 mb-4 flex items-center">
                    ...
                    <div className="space-y-4">
                      {sortedWorks.filter(w => w.type === 'oral').map(work => (
                        <div key={work.id} className="flex flex-col sm:flex-row sm:items-start justify-between p-4 border border-slate-200 rounded-xl bg-slate-50 relative">
                          <div className="mb-4 sm:mb-0 pr-4">
                            <div className="flex items-center gap-2 mb-1">
  ...
  code = code.substring(0, startIndex) + newWorksUi + code.substring(endIndex);
  
  // Also add states for filters at the beginning of the component
  code = code.replace(
    "const [showAddWork, setShowAddWork] = useState(false);",
    "const [showAddWork, setShowAddWork] = useState(false);\n  const [filterDate, setFilterDate] = useState('');\n  const [filterTematica, setFilterTematica] = useState('');"
  );
  
  // Add sortedWorks computation
  const sortedWorksLogic = `  const sortedWorks = localWorks
  ...
  console.log("Successfully patched works UI");
} else {
