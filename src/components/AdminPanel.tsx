...
export function AdminPanel({ posters, assignments, evaluations, criteria, evaluators = [], onSaveAssignments, onSaveCriteria, onSavePosters, onSaveEvaluators, onClearEvaluation, onLogout }: AdminPanelProps) {
  ...
  const handleEditWorkClick = (work: Poster) => {
    ...
    setEditWorkAuthors(work.authors || '');
  ...
  const handleExportResults = (type: 'oral' | 'poster', tematica: Tematica) => {
    const rows = posterStats.filter(s => s.type === type && s.tematica === tematica).map(s => ({
      ID: s.posterId,
      ...
      Título: s.title,
      Autores: s.authors || '',
      Apresentador: s.presenterName,
      Status: s.absent ? 'Ausente' : 'Apresentado',
      Avaliadores: s.evaluatorNames.join(', '),
      'Nº avaliações': s.evalCount,
      'Nota média': Number(s.averageScore.toFixed(2)),
      Comentários: s.comments.join(' | ')
    }));
        ...
        {editingWorkId === work.id && (
          <div className="mt-4 pt-4 border-t border-slate-200 space-y-3" onClick={(e) => e.stopPropagation()}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <input value={editWorkAuthors} onChange={(e) => setEditWorkAuthors(e.target.value)} placeholder="Autores" className="px-3 py-2 border border-slate-200 rounded-lg text-sm" />
              ...
              <div className="mt-4 pt-2">
                ...
                {showAddWork && (
                  <form onSubmit={(e) => { handleAddWork(e); setShowAddWork(false); }} className="flex flex-col gap-4 mt-4 p-4 border border-slate-200 rounded-xl bg-white shadow-sm">
                    ...
                    <input type="text" value={newWorkAuthors} onChange={(e) => setNewWorkAuthors(e.target.value)} placeholder="Autores" className="px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-teal-500 outline-none text-slate-900 transition-colors" />
